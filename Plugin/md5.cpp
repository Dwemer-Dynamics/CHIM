...
std::string md5(const std::string str,bool trim) {
    ...
    MD5 md5 = MD5(s);
    std::string md5s = md5.hexdigest();
    logger::info("HASH for <{}> <{}>", s, md5s);

    return md5s;
}

std::string md5low(const std::string str, bool trim) {
    std::string s = str;
    if (trim) {
        // Trim leading whitespace
        s.erase(0, s.find_first_not_of(" \t\n\r\f\v"));
    ...
    MD5 md5 = MD5(s);
    std::string md5s = md5.hexdigest();
    logger::info("HASH LOW for <{}> <{}>", s, md5s);
