...
function ResetPackages(Actor npc) global
	...
	StorageUtil.FormListRemove(None, "CHIM_ComeCloserPending", npc, true)
	StorageUtil.UnsetFormValue(npc, "CHIM_ComeCloserTarget")
	StorageUtil.UnsetFloatValue(npc, "CHIM_ComeCloserStartedAt")
...
function ComeCloser(Actor npc, ObjectReference akTarget) global
	...
	StorageUtil.SetFormValue(npc, "CHIM_ComeCloserTarget", akTarget)
	StorageUtil.SetFloatValue(npc, "CHIM_ComeCloserStartedAt", Utility.GetCurrentRealTime())
	StorageUtil.FormListAdd(None, "CHIM_ComeCloserPending", npc, false)

endFunction

; Processes one queued approach without holding a Papyrus stack between checks.
function ProcessComeCloserRestore() global

	Actor npc = StorageUtil.FormListShift(None, "CHIM_ComeCloserPending") as Actor
	if (!npc)
		return
	endif

	ObjectReference target = StorageUtil.GetFormValue(npc, "CHIM_ComeCloserTarget", None) as ObjectReference
	float startedAt = StorageUtil.GetFloatValue(npc, "CHIM_ComeCloserStartedAt", 0.0)
	if (StorageUtil.GetIntValue(npc, "CHIM_FollowPlayerActive", 0) != 1 || !target)
		StorageUtil.UnsetFormValue(npc, "CHIM_ComeCloserTarget")
		StorageUtil.UnsetFloatValue(npc, "CHIM_ComeCloserStartedAt")
		return
	endif

	float elapsedSeconds = Utility.GetCurrentRealTime() - startedAt
	if (elapsedSeconds < 0.0)
		elapsedSeconds = 20.0
	endif
	float distance = npc.GetDistance(target)
	if (distance > 300.0 && elapsedSeconds < 20.0)
		StorageUtil.FormListAdd(None, "CHIM_ComeCloserPending", npc, false)
		return
	endif
